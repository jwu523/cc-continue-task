...
- 可以作为 Codex skill 使用，也可以把 handoff schema 和辅助脚本用于 Claude Code、OpenCode 等文件型代理。
- 保存可验证的 handoff 文件，并输出一段可直接复制到新对话的恢复提示。
- 保存前执行质量门槛；不确定是否能省略或压缩的细节，先让用户确认。

典型流程：保存 handoff -> 复制生成的恢复提示 -> 开启新对话 -> 从可验证状态恢复 -> 下次保存时继续保持 Original Objective 稳定。
